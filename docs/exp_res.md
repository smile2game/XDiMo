...
**衡量方式**：采用「**迭代相同数据量的加速比**」—— 固定训练步数（如 **10k steps**），比较单卡完成耗时与多卡完成耗时，加速比 = 单卡耗时 ÷ 多卡耗时（>1 表示多卡更快）。

以下三张表分别对应硬件 **6000ada**、**H200**、**pro6000**，表头一致。

### 表一-A：6000ada 

| 模型 | 卡数 | 并行方式 | Steps/s | Samples/s | MFU | 完成 10k steps 耗时(s) | 加速比(相对1卡) | 理想加速比 | 并行效率(%) |
|------|------|----------|---------|-----------|-----|------------------------|-----------------|------------|-------------|
| Latte-L/2-MoE | 1×6000ada | 单卡 | 1.46 | 1.46 | 87.5% | 6849 | 1.0(基准) | 1 | 100% |
| Latte-L/2-MoE | 2×6000ada | DDP 单机 | 1.25 | 2.50 | 74.5% | 8000 | 0.86 | 2 | 43% |
| Latte-L/2-MoE | 4×6000ada | DDP 单机 | 1.15 | 4.60 | 68.5% | 8696 | 0.79 | 4 | 20% |
...
- **完成 10k steps 耗时(s)**：在「相同数据量」（固定 10k steps）下该配置的耗时，用于计算加速比。
- **加速比(相对1卡)**：= 同硬件单卡完成 10k steps 的耗时 ÷ 本配置完成 10k steps 的耗时。
- **理想加速比**：= 卡数。
- **并行效率(%)**：= 实际加速比 ÷ 理想加速比 × 100%。

**表下说明：**

> **加速比定义**：本表采用「迭代相同数据量」的加速比。即固定训练步数（如 10k steps），单卡跑完所需时间为 T₁，多卡跑完所需时间为 Tₙ，则加速比 = T₁ / Tₙ。等价地，也可用吞吐比：加速比 = (多卡 Steps/s) / (单卡 Steps/s)，前提是各配置的 global_batch 一致；若 batch 不同，需用「完成相同总样本数」的耗时比。
