...
def get_frame_indices(num_frames: int, total_frames: int, sample: str = "middle") -> list:
    """
    Generate frame indices with boundary protection
    :param num_frames: number of frames to sample
    :param total_frames: total frames in video
    :param sample: sampling method ('middle', 'uniform', etc.)
    ...
    if sample == "middle":
        ...
        raise NotImplementedError(f"Sampling method {sample} not implemented")
    
    # 确保所有索引在[0, total_frames-1]范围内
