...
class FaceForensics(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        # 至少需要 num_frames * frame_interval 帧（temporal_sample 的 size）
        min_frames = self.target_video_len * getattr(
            self.configs, 'frame_interval', 3
        )
        for _ in range(3):  # 最多重试 3 次，避免短视频导致 assert
            path = self.video_lists[index % len(self.video_lists)]
            vframes, aframes, info = torchvision.io.read_video(filename=path, pts_unit='sec', output_format='TCHW')
            total_frames = len(vframes)
            if total_frames < min_frames:
                index = (index + 1) % len(self.video_lists)
                continue
        ...
        # 若多次仍失败，取前 target_video_len 帧（短视频兜底）
        path = self.video_lists[index % len(self.video_lists)]
        vframes, aframes, info = torchvision.io.read_video(filename=path, pts_unit='sec', output_format='TCHW')
        total_frames = len(vframes)
        frame_indice = np.linspace(0, max(0, total_frames - 1), self.target_video_len, dtype=int)
        video = vframes[frame_indice]
